...
class ReactionFactoryTestCase(unittest.TestCase):
    ...
    def test_PEN_getReactionList(self):  #例外処理（Inhibitor,self-start?)
        ...
        self.assertTrue(rlist1[1][0].products[0] is str(edge0.template)+"_in")
        self.assertTrue(rlist1[1][1].reactants[0] is A)
        self.assertTrue(rlist1[1][1].reactants[1] is G1)
        self.assertTrue(rlist1[1][1].products[0] is str(edge0.template)+"_out")
        self.assertTrue(rlist1[1][2].reactants[0] is A)
        self.assertTrue(rlist1[1][2].reactants[1] is str(edge0.template)+"_out")
        self.assertTrue(rlist1[1][2].products[0] is str(edge0.template)+"_both")
        self.assertTrue(rlist1[1][3].reactants[0] is A)
        self.assertTrue(rlist1[1][3].reactants[1] is str(edge0.template)+"_in")
        self.assertTrue(rlist1[1][3].products[0] is str(edge0.template)+"_both")
        self.assertTrue(rlist1[1][4].reactants[0] is str(edge0.template)+"_in")
